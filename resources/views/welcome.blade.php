...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <!----------------------------- Cor do fundo e espacamentos: -------------->
        <div class="relative flex items-top justify-center min-h-screen sm:items-center py-4 sm:pt-0">
            
            @if (Route::has('login'))
                <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                    @auth
                        <a href="{{ url('/dashboard') }}" class="ml-4 .dark\:text-preto underline">Sua área</a>
                    @else
                        <a href="{{ route('login') }}" class="ml-4 .dark\:text-preto underline">Login</a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="ml-4 .dark\:text-preto underline">Cadastre-se</a>
                        @endif
                    @endauth
                </div>
            @endif
